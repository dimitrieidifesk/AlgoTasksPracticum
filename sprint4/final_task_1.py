...
Удаление происходит поиском баскета и заменой параметра is_deleted на True.

Удаление заменой баскета на None сделать нельзя, потому что при разрешении коллизий при поиске элемента мы можем наткнуться на
удаленный элемент, замененный на None и в таком случае из метода _get_index вернется None, в то время как искомый элемент
находился дальше в одном из следующеих баскетах.
...
В худшем случае для разрешения коллизий нам понадобится обойти соседние баскеты вплоть до обхода всех элементов хеш-таблицы.
Суммарное время работы программы O(1) в среднем
